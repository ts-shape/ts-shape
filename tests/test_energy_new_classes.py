...
from ts_shape.events.energy.energy_performance_indicator import EnergyPerformanceIndicatorEvents
...
def _make_df(hours: int = 48) -> pd.DataFrame:
    ...
    energy_vals = np.where(is_production, 50.0 + np.random.default_rng(0).normal(0, 5, n), 5.0)
    counter_vals = np.where(is_production, 100, 0).cumsum()
    state_vals = is_production.astype(bool)

    rows = []
    for i, ts in enumerate(idx):
        rows.append({"systime": ts, "uuid": "meter:main", "value_double": energy_vals[i], "is_delta": True})
        rows.append({"systime": ts, "uuid": "counter:line1", "value_integer": int(counter_vals[i]), "is_delta": True})
        rows.append({"systime": ts, "uuid": "state:machine1", "value_bool": state_vals[i], "is_delta": True})
...
def _make_csv_df(hours: int = 48) -> pd.DataFrame:
    ...
    hour_of_day = idx.hour
    is_production = (hour_of_day >= 6) & (hour_of_day < 22)
    energy_vals = np.where(is_production, 50.0 + np.random.default_rng(1).normal(0, 5, n), 5.0)

    rows = [{"time": ts, "id": "meter:main", "value": energy_vals[i]} for i, ts in enumerate(idx)]
    return pd.DataFrame(rows)
...
class TestNormalizeStatic:
    ...
    def test_id_column_overrides_series_id(self):
        ...
        out = EnergyConsumptionEvents.normalize(raw, series_id="ignored", id_column="id")
...
class TestUuidColumnAgnosticism:
    def test_consumption_with_id_column(self):
        ...
        result = events.consumption_by_window("meter:main", value_column="value", window="1h")
    ...
    def test_efficiency_with_id_column(self):
        ...
        result = events.specific_energy_consumption(
            "meter:main", "counter:line1",
            energy_column="value", counter_column="value",
            window="1h",
        )
        assert not result.empty
...
class TestIdleEnergyDetectionEvents:
    ...
    def test_trend_direction_values(self):
        ...
        assert set(result["trend_direction"].dropna().unique()).issubset({"improving", "stable", "worsening"})
    ...
    def test_raw_csv_model_path(self):
        ...
        idx = pd.date_range("2026-01-13", periods=16, freq="15min", tz="UTC")
        state_rows = [{"time": ts, "id": "state:machine1", "value": float(ts.hour >= 6 and ts.hour < 22)} for ts in idx]
        ...
        result = events.idle_energy_by_window(
            "meter:main", "state:machine1",
            energy_column="value", state_column="value",
        )
        assert not result.empty
...
class TestEnergyPerformanceIndicatorEvents:
    ...
    def test_raw_csv_model_path(self):
        ...
        for i, ts in enumerate(idx):
            ...
            rows.append({"time": ts, "id": "counter:line1", "value": float(i * 100)})
        df = pd.DataFrame(rows)
        events = EnergyPerformanceIndicatorEvents(df, time_column="time", uuid_column="id")
...
def _add_gas_signal(df: pd.DataFrame) -> pd.DataFrame:
    ...
    gas_rows = [{"systime": ts, "uuid": "meter:gas", "value_double": 10.0, "is_delta": True} for ts in timestamps]
...
class TestCarbonIntensityEvents:
    ...
    def test_total_emissions_sums_scopes(self):
        ...
        assert (total["total_kgco2e"] == total["scope1_kgco2e"] + total["scope2_kgco2e"]).all()

    def test_carbon_intensity_per_unit(self):
        events = CarbonIntensityEvents(self.df, {"meter:main": 0.233}, scope_map={"meter:main": 2})
